...
import com.cotowork.userservice.exception.ErrorCode; // Import ErrorCode
import com.cotowork.userservice.exception.ResourceNotFoundException;
import com.cotowork.userservice.exception.DuplicateDataException; // Sửa tên Exception cho khớp
import com.cotowork.userservice.exception.ValidationException; // Thêm ValidationException
...
public class UserService {
    ...
    public UserResponseDto createUser(UserCreateDto dto) {
        ...
        // ✅ MỚI: (dùng convenience method có sẵn trong class)
        if (userRepository.existsByUsername(dto.getUsername())) {
            throw DuplicateDataException.duplicateUsername(dto.getUsername());
        }
        if (userRepository.existsByEmail(dto.getEmail())) {
            throw DuplicateDataException.duplicateEmail(dto.getEmail());
        }
    ...
    public List<UserResponseDto> getUsersByUnit(Long unitId) {
        ...
        List<User> users = userRepository.findAll().stream()
            .filter(user -> user.getUnit() != null && user.getUnit().getId().equals(unitId))
            .toList();
        return userMapper.toResponseDtoList(users);
    }
